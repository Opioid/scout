...
func (p *Provider) Load2D(filename string, config Config) *Texture2D {
	...
	if file.RGBE == fileType {
		...
		return textureFromGoSupportedFile(fi, config)
	}
}
...
func textureFromGoSupportedFile(fi *os.File, config Config) *Texture2D {
